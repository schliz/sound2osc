...
#else
	// In Qt6, QVector is an alias for QList, so fromVector() is just fromList()
	static QCircularBuffer<T> fromVector(const QVector<T> &vector)
...
#else
	// In Qt6, QVector is an alias for QList, so toVector() is just toList()
	QVector<T> toVector() const { return toList(); }
#endif
